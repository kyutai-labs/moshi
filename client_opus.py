...
async def main():
    ...
    async def recv_loop():
        ...
        while True:
            try:
                ...
                printer.log("error", "Lost connection with the server!")
                done = True
                return
            if not isinstance(message, bytes):
                printer.log("warning", f"unsupported message type {type(message)}")
                continue
            if len(message) == 0:
                printer.log("warning", "empty message")
                continue
            kind = message[0]
            if kind == 1:  # audio
                payload = message[1:]
                ...
                printer.log("warning", f"unknown message kind {kind}")
    ...
    try:
        connect = websockets.connect(uri)
        websocket = await connect.__aenter__()
    except websockets.exceptions.WebSocketException:
