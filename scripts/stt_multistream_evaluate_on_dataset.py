...
#With multistream model

# LibriSpeech clean === cer: 1.51% wer: 3.88% corpus_wer: 3.26% RTF = 48.01
# LibriSpeech other === cer: 5.59% wer: 11.05% corpus_wer: 9.18% RTF = 47.81
# voxpopuli === cer: 6.46% wer: 12.29% corpus_wer: 11.38% RTF = 56.18


# Tedlium Short === cer: 3.11% wer: 5.54% corpus_wer: 4.46% RTF = 51.67
# Gigaspeech cer === 
# spgispeech cer === cer: 2.59% wer: 5.18% corpus_wer: 5.08% RTF = 58.12
# earning 22 short === 
# AMI === 
...
def run_inference(
    ...
    for i in tqdm.tqdm(range(0, len(dataset), args.batch_size)):
        ...
        for batch_index in range(text_tokens.shape[0]):
            ...
            if len(utterance_tokens.shape) == 2: # multistream
                utterance_tokens = utterance_tokens.transpose(0,1)
                utterance_tokens = utterance_tokens.reshape(-1)
            utterance_tokens = utterance_tokens[utterance_tokens > 4]
...
def main(args):
    ...
    lm_gen = moshi.models.LMGen(lm, temp=0, temp_text=0.0,condition_tensors={})
