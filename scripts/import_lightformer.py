...
def import_model(
    ...
    for idx in range(cfg.transformer_lm.depformer_num_layers):
        ...
        in_proj = in_proj.view(num_weights, *in_proj.shape[1:])
        ...
        out_proj = out_proj.view(num_weights, *out_proj.shape[1:])
        model[in_proj_key] = in_proj[:kept_weights].view(-1, *in_proj.shape[2:]).contiguous()
    ...
    for dep_idx in range(out_n_q, in_n_q):
        ...
        real_idx = schedule[dep_idx]
        model.pop(f"depformer_in.{real_idx}.weight", None)
        for idx in range(DEPFORMER_LAYERS):
            model.pop(f"depformer.layers.{idx}.gating.{real_idx}.linear_in.weight", None)
            model.pop(f"depformer.layers.{idx}.gating.{real_idx}.linear_out.weight", None)
