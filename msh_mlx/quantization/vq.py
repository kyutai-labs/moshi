...
class ResidualVectorQuantizer(BaseQuantizer):
    ...
    def forward(self, x: mx.array, frame_rate: int):
        """
        Args:
            x (mx.array): Input tensor of shape [B, C, T] with `C` number of channels.
        ...
        Returns:
            QuantizedResult: Quantized result with the following attributes:
                - `x` (mx.array): Quantized tensor of shape [B, C, T].
                - `codes` (mx.array): Quantized codes of shape [B, K, T] with `K` number of codebooks.
                - `bw` (mx.array): Bandwidth of the quantized tensor in kbits per second.
                - `penalty` (mx.array): Commitment loss.
        ...
        return QuantizedResult(
            quantized, codes, bw, penalty=mx.mean(commit_loss), metrics=metrics
        )
    ...
    def encode(self, x: mx.array) -> mx.array:
        ...
        n_q = self.n_q
        if x.shape[-1] == 0:
            return mx.zeros((x.shape[0], n_q, 0), dtype=mx.int64)
...
class SplitResidualVectorQuantizer(BaseQuantizer):
    ...
    def forward(self, x: mx.array, frame_rate: int):
        ...
        full_quantized_codes = mx.concat(
    ...
    def encode(self, x: mx.array) -> mx.array:
        ...
        if self.n_q > self.n_q_semantic:
            ...
            codes = mx.concat([codes, acoustic_codes], axis=1)
        # codes is [B, K, T], with T frames, K nb of codebooks.
        return codes
