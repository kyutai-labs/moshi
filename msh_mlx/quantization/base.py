...
class QuantizedResult:
    x: mx.array
    codes: mx.array
    bandwidth: mx.array  # bandwidth in kb/s used, per batch item.
    penalty: tp.Optional[mx.array] = None
...
class BaseQuantizer(nn.Module):
    ...
    def forward(self, x: mx.array, frame_rate: int) -> QuantizedResult:
        """
...
class DummyQuantizer(BaseQuantizer):
    ...
    def forward(self, x: mx.array, frame_rate: int):
        ...
        return QuantizedResult(x, q, mx.array(q.size * 32 * frame_rate / 1000 / len(x)).to(x))
