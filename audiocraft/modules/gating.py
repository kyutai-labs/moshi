...
class SVDGating(nn.Module):
    ...
    def __init__(self, dim: int, activation, small_init: bool = True, tanh_rot: bool = False,
                 tanh_scale: float = 1, scale_rot: float = 0.1, **factory_kwargs):
        super().__init__()
...
def _make_gating(name: str, dim: int, dim_feedforward: int, **factory_kwargs) -> nn.Module:
    if name.startswith('svd_'):
        ...
        return ActivationGating(dim, dim_feedforward, _get_activation('silu'), **factory_kwargs)
    else:
