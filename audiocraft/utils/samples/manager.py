...
class SampleManager:
    ...
    def load_samples(self):
        ...
        with ThreadPoolExecutor(6) as pool:
            self.samples = [x for x in pool.map(self._load_sample, jsons) if x is not None]
        return self

    @staticmethod
    @lru_cache(2**26)
    def _load_sample(json_file: Path) -> tp.Optional[Sample]:
        with open(json_file, 'r') as f:
            try:
                data: tp.Dict[str, tp.Any] = json.load(f)
            except json.decoder.JSONDecodeError as exc:
                logger.warning("Failed to parse %s: %r", json_file, exc)
                return None
        # fetch prompt data
...
def get_samples_for_xps(xps: tp.List[dora.XP], **kwargs) -> tp.Dict[str, tp.List[Sample]]:
    ...
    * Unstable IDs are of the form "noinput_{idx}" and are generated on-the-fly, in order to map samples
    ...
    Args:
        ...
        exclude_unconditioned (bool): If True, excludes samples that did not use conditioning.
    """
    managers = [SampleManager(xp).load_samples() for xp in xps]
